...
type Props = {
	...
	applications: AllApplicationsByActivityQuery['allApplications'];
};

type ApprovalStatus = 'APPROVED' | 'DENIED' | 'PENDING';

export default function ActivityAdmin({ applications: _applications }: Props) {
	...
	const updateStatus = async (id: string | null, approvalStatus: ApprovalStatus) => {
		setLoading(true);
		...
		try {
			...
			if (res.status !== 200) return setError('Något gick fel, försök igen senare');

			await res.json();
		} catch (e) {
			if ((e as AbortController['signal']['reason']).name === 'AbortError') return;
			setLoading(false);
			throw e;
		}
	...
	const handleApprove = (e: React.MouseEvent) => {
		...
		setApplications((applications) =>
			applications.map((a) => {
				if (a.id === applicationId) a.approvalStatus = approval;
				return a;
			}),
		);

		setError(null);
		updateStatus(applicationId, approval)
			.then(() => {
				//console.log('success');
			})
			.catch((e) => {
	...
	const handleExport = async () => {
		...
		const t = applications
			...
			.map(({ member }) => columns.map((c) => member[c as keyof typeof member]).join('\t'))
			.join('\n');
			...
			.map(({ member }) =>
				columns
					.map((c) => {
						...
						return member[c as keyof typeof member] || '';
					})
					.join('\t'),
			)
			.join('\n');
	...
	function Application({
		application: { id, approvalStatus, member },
		...
		return (
			<>
				...
				>
					...
					<td className={s.email}>{member.email}</td>
					<td>{member.sex}</td>
					<td>{member.age}</td>
					...
					<td className={s.buttons}>
						...
						<button
							...
							onClick={handleApprove}
						>
							{approvalStatus !== 'PENDING' ? 'Ångra' : 'Utvald'}
						</button>
					</td>
				</tr>
			</>
	...
	return (
		<>
			<table className={s.container}>
				<tbody>
					<tr>
						<th colSpan={colSpanMax}>Ohanterade anmälningar ({pending.length})</th>
					</tr>
					<tr>
					...
					{pending.map((application, i) => (
						<Application key={i} application={application as ApplicationRecord} />
					))}
					{!pending.length && (
						<tr>
							<td>Alla anmälningar hanterade</td>
						</tr>
					)}

					<tr>
						<th colSpan={colSpanMax}>Utvalda ({approved.length})</th>
					</tr>
					<tr>
						<td colSpan={colSpanMax}>
							<hr />
					...
					{approved.map((application, i) => (
						<Application key={i} application={application as ApplicationRecord} />
					))}
					{!approved.length && (
						<tr>
							<td>Inga anmälningar är utvalda</td>
						</tr>
					)}

					<tr>
						<th colSpan={colSpanMax}>Bortvalda ({declined.length})</th>
					</tr>
					<tr>
						<td colSpan={colSpanMax}>
							<hr />
					...
					{declined.map((application, i) => (
						<Application key={i} application={application as ApplicationRecord} />
					))}
					{!declined.length && (
						<tr>
							<td>Inga anmälningar är bortvalda</td>
						</tr>
					)}

					<tr>
						<td colSpan={colSpanMax}>{error && <p className={s.error}>{error}</p>}</td>
					</tr>
				</tbody>
			</table>
			<button
				className={cn('wide', s.export)}
			...
			<button
				className={cn('wide', s.export)}
				onClick={handleDeclinedExport}
				disabled={approved.length === 0}
			>
				Kopiera mailadresser för bortvalda
			</button>
			<button
				className={cn('wide', s.export, s.exportFirst)}
				onClick={handleExport}
				disabled={approved.length === 0}
			>
				Kopiera mailadresser för utvalda
			</button>
				...
				<Modal>
					<div className={cn(s.modal, application && s.show)}>
						<div className={s.wrap}>
							<div className={s.content}>
								...
								<div className={s.cols}>
									<span>
										<h5>Kontakt:</h5>
										<p>
											{application.member?.email}, {application.member?.phone}
										</p>
									</span>
									<span>
										<h5>Adress:</h5>
									...
									<span>
										...
										{application.member?.url && (
											<p>
												<a href={application.member?.url} rel='noreferrer' target='_new'>
													Besök
												</a>
											</p>
										)}
									...
									<span>
										<h5>
											Jag har arbetat längre än 3 år som professionell konstnär utan att ha gått
								...
								<div className={s.buttons}>
									<button
										...
										data-approval={
											application.approvalStatus === 'PENDING' ? 'APPROVED' : 'PENDING'
										}
									...
									<button
										data-application-id={application.id}
										data-approval={
											application.approvalStatus !== 'DECLINED' ? 'DECLINED' : 'PENDING'
										}
							...
							<button className={s.close} onClick={() => setApplication(null)}>
								Stäng
							</button>
						</div>
					</div>
