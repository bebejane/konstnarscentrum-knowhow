'use client';

import s from './Image.module.scss';
import { useEffect } from 'react';
import { Image as DatoImage } from 'react-datocms';
import { ImageGallery } from '@/components';
import { Markdown } from 'next-dato-utils/components';
import { useStore, useShallow } from '@/lib/store';

export type ImageBlockProps = {
	id: string;
	data: ImageRecord;
	onClick: Function;
	editable?: any;
};

export default function Image({ id, data: { image: images, layout }, editable }: ImageBlockProps) {
	const [imageId, setImageId, setImages] = useStore(
		useShallow((state) => [state.imageId, state.setImageId, state.setImages]),
	);

	const isSingle = images?.length === 1;
	const isDouble = images?.length === 2;
	const isGallery = images?.length > 2;

	useEffect(() => {
		setImages(!images ? [] : images);
	}, [images, setImages]);

	if (!images || !images.length) return null;

	return isSingle ? (
		images[0].responsiveImage && (
			<figure
				className={s.single}
				onClick={() => setImageId(images[0].id)}
				data-editable={editable}
			>
				<DatoImage
					data={images[0].responsiveImage}
					className={s.image}
					intersectionMargin='0px 0px 200% 0px'
				/>
				{images[0].title && (
					<figcaption>
						<Markdown allowedElements={['em', 'p']} content={images[0].title} />
					</figcaption>
				)}
			</figure>
		)
	) : isDouble ? (
		<div className={s.double} data-editable={editable}>
			<div className={s.imgWrap}>
				{images[0].responsiveImage && (
					<figure onClick={() => setImageId(images[0].id)}>
						<DatoImage
							data={images[0].responsiveImage}
							className={s.image}
							intersectionMargin='0px 0px 200% 0px'
						/>
					</figure>
				)}
				{images[1].responsiveImage && (
					<figure onClick={() => setImageId(images[1].id)}>
						<DatoImage
							data={images[1].responsiveImage}
							className={s.image}
							intersectionMargin='0px 0px 200% 0px'
						/>
					</figure>
				)}
			</div>

			<div className={s.captionWrap}>
				{images[0].title && (
					<figcaption>
						<Markdown allowedElements={['em', 'p']} content={images[0].title} />
					</figcaption>
				)}
				{images[1].title && (
					<figcaption>
						<Markdown allowedElements={['em', 'p']} content={images[1].title} />
					</figcaption>
				)}
			</div>
		</div>
	) : isGallery && layout === 'grid' ? (
		<ul className={s.grid}>
			{images.map(
				(image) =>
					image.responsiveImage && (
						<li key={image.id} onClick={() => setImageId(image.id)}>
							<figure>
								<DatoImage
									data={image.responsiveImage}
									className={s.image}
									intersectionMargin='0px 0px 200% 0px'
								/>
							</figure>
						</li>
					),
			)}
		</ul>
	) : isGallery ? (
		<ImageGallery id={id} images={images} />
	) : null;
}
