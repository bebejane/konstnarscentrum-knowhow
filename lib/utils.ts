...
export const getStaticPaginationPaths = async (query: TypedDocumentNode, segment: string) => {
...
export const getStaticPagePaths = async (query: TypedDocumentNode, segment: string) => {
  const items = await fetchAllRecords(query)
  const paths = []
