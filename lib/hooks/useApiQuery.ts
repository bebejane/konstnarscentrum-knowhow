...
const useApiQuery = <T>(
	...
	{ variables, initialData, pageSize }: UseApiQueryProps = {},
) => {
		...
		pageSize
			? {
					...
					size: pageSize,
						...
						initialData?.pagination?.count > 0 ? initialData?.pagination?.count <= pageSize : false,
				}
			: undefined,
	);
	...
	const mergeData = (newData, oldData) => {
		...
		Object.keys(newData).forEach((k) => {
			if (oldData[k] && Array.isArray(oldData[k])) newData[k] = [...oldData[k], ...newData[k]];
		});
		return newData;
	};
