...
export default function Knowledges({
	category,
	presentKnowledges,
	knowledges: knowledgesFromProps,
	knowledgeCategories,
	date,
	pagination,
}: Props) {
	const {
		...
		nextPage,
		page,
	} = useApiQuery<{ knowledges: KnowledgeRecord[] }>(AllPastAndFutureKnowledgesDocument, {
		...
		<>
			...
			>
				{allKnowledges.length > 0 ? (
					...
					<div className={s.nomatches}>Mer information kommer här i augusti.</div>
				)}
			</CardContainer>
...
export const getStaticProps: GetStaticProps = withGlobalProps(
	{ queries: [AllKnowledgeCategoriesDocument] },
	async ({ props, revalidate, context }: any) => {
		...
		let { knowledges, pagination } = await apiQueryAll(AllPastAndFutureKnowledgesDocument, {
			variables: { date, categoryId },
		});
