...
export const sendVerificationRequest = async (params: any) => {
	...
	try {
		const member = (
			await client.items.list({ page: { limit: 1 }, filter: { type: 'member', fields: { email: { eq: email } } } })
		)?.[0];
		if (!member) throw new Error('Du är ej registrerad som medlem än. Var god fyll i formuläret nedan.');
